...
/// The 10 SEO component keys in canonical display order (LLD §7.2, matches
/// `scoring::seo::SeoComponents::values`).
pub const SEO_COMPONENT_KEYS: [&str; 10] = [
    "keyword_title",
    "title_front",
    "title_length",
    "title_hooks",
...
async fn alerts_page(State(st): State<AppState>) -> Response {
    ...
    let list_html = match render(AlertsListTemplate {
        alerts: &alerts,
        unread,
    }) {
        Ok(s) => s,
...
/// return the fresh list fragment.
async fn alerts_read(State(st): State<AppState>, headers: HeaderMap) -> Response {
    ...
    alerts_list_fragment(&st).await
}

/// POST /alerts/clear: delete all alerts (CLI `alerts clear` path), then
/// return the (now empty) list fragment.
...
async fn health_page(State(st): State<AppState>) -> Response {
    ...
    let index_last = h["index"]["last_reindex_at"].as_str().unwrap_or("—").to_string();
    let body = match render(HealthTemplate {
        counts: &counts,
        quota_used: &h["quota"]["videos_list_used"].as_u64().unwrap_or(0).to_string(),
...
async fn score_rows(
    ...
    let score_by_id: HashMap<&str, &crate::storage::db::ScoreRow> = scores
        .iter()
        ...
        .map(|v| {
            ...
            ScoreRowView {
                ...
                category: v
                    ...
                    .unwrap_or_else(|| "—".to_string()),
                seo: s.map(|s| fmt_score(s.seo_score)).unwrap_or_else(|| "—".to_string()),
                geo: s.map(|s| fmt_score(s.geo_score)).unwrap_or_else(|| "—".to_string()),
                total: s.map(|s| fmt_score(s.total_score)).unwrap_or_else(|| "—".to_string()),
                has_score,
            }
        })
        .collect();
...
fn scorecard_view(c: &Value, is_median: bool) -> ScorecardRowView {
    ...
    ScorecardRowView {
        ...
        views_growth: c["views_growth"].as_f64().map(|v| format!("{v:.2}")).unwrap_or_else(|| "—".to_string()),
...
mod tests {
    ...
    #[test]
    fn component_keys_cover_17() {
        assert_eq!(SEO_COMPONENT_KEYS.len(), 10);
