...
pub struct ScoreResult {
    ...
    /// The full SEO component struct (Phase 6.6 — feeds the recommendation
    /// checklist, which needs the typed values, not just the JSON map).
    pub seo_struct: seo::SeoComponents,
}
...
/// Full scoring pass with graph signals. The three graph-based SEO components
/// (tag_authority, topic_dominance, keyword_competition) are computed only
/// when the corresponding graph data is provided — otherwise they score 0.
///
...
pub fn compute_with_graph(
    ...
    exclude_video_id: Option<&str>,
    geo_meta: &geo::GeoMeta,
    tag_authority_scores: Option<&std::collections::HashMap<String, f64>>,
    topic_dominance_scores: Option<&std::collections::HashMap<(String, String), f64>>,
    kw_channel_edges: Option<&[crate::analytics::graph::KwChannelEdge]>,
    channel_id: Option<&str>,
) -> ScoreResult {
    let eff = effective_keywords(keywords, title);
    let seo_c = seo::compute_with_graph(
        title,
        desc,
        tags,
        &eff,
        bm25,
        exclude_video_id,
        tag_authority_scores,
        topic_dominance_scores,
        kw_channel_edges,
        channel_id,
    );
...
pub async fn score_video(
    ...
    // Build graph signals for scoring
    let videos = db.all_videos().await?;
    let centrality = crate::analytics::graph::build(db, &videos).await?;
    let tag_authority = crate::analytics::graph::tag_authority_scores(&videos, &centrality);
    let topic_dominance = crate::analytics::graph::topic_dominance_scores(&videos);
    let kw_channel_edges = crate::analytics::graph::build_kw_channel_graph(&videos, &keywords);

    let r = compute_with_graph(
        &video.title,
        &video.description,
        &tags,
        &keywords,
        ...
        &meta,
        Some(&tag_authority),
        Some(&topic_dominance),
        Some(&kw_channel_edges),
...
fn weighted(items: &[(&'static str, f64)], w: &Weights) -> f64 {
    ...
    let denom: f64 = items
        .iter()
        .map(|(k, _)| w.seo_weight(k) + w.geo_weight(k))
        .sum();
    if denom <= 0.0 {
        0.0
    } else {
...
/// SEO weighted total for an arbitrary component list (used by the keyword
/// research SERP overlay to score competitor metadata).
pub fn seo_total(items: &[(&'static str, f64)], w: &Weights) -> f64 {
    round2(weighted(items, w).clamp(0.0, 100.0))
}
